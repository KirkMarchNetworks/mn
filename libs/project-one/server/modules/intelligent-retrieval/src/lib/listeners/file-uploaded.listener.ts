...
import { generateFileId, generateId, MultiUploadRequest } from '@mn/project-one/server/models';
...
const multiUploadEventName = new MultiUploadRequest({ uploadName: MultiUploadRootName }).getEventName();
...
export class FileUploadedListener {
  ...
  async handleFileUploadedEvent({ params }: FileUploadedEvent) {
    ...
    const { tenantId, path, bucketName, originalName, customMetadata } = params.metadata;
    const fileName = generateFileId();

    await this.fileService.moveObject(bucketName, path, 'imageBucket', convertFileNameToStoragePath(tenantId, fileName));

    const data = intelligentRetrievalParse(customMetadata);

    if (data) {
      const entity = await this.repo.addImage(data.channelId, fileName, originalName);
